class Paddle {
  ...
  Paddle() {
    ...
    pos = new float[]{width/6, 31*height/33-lowBound};
    vel = new float[]{0, 0};
    ace = new float[]{0, 0};
    speed = 3;
    size = height/33;
  ...
  void calculate() {
    ...
    //SlamDown
      ...
      if(blastTime>frameCount){
        Blast.drawAni(blastPos[0],blastPos[1],floor((blastTime-frameCount+2)/3),false,5*PI/4);
      }
    ...
    //ForceWall
      ...
      if(forceTime>frameCount){
        fill(0,255,255,float(forceTime-frameCount)/15*255);
        rectMode(CENTER);
  ...
  void drawBarrier() {
    ...
    if (curFrame>frameCount) {
      for (int i =0; i<floor(width/30); i++) {
        fill(0, 255, 255, (float(floor(width/30)-i-1))/floor(width/30)*255*(curFrame-frameCount)/10);
        rectMode(CORNERS);
        noStroke();
  ...
  float[] collision(float[] ballParams){
    ...
    if(move[3]){
      ...
      if(ballParams[1]>pos[1]-size/2 && ballParams[1]<pos[1]+size/2){
        if(ballParams[0]>pos[0]-(forceTime-frameCount-15)*15-size*4 && ballParams[0]<pos[0]-(forceTime-frameCount-15)*15+size*4 ){
      ...
      if(!airbourne){
        if((distance*cos(angle)<maxHypo*cos(maxAngle) && abs(angle)<maxAngle) || (abs(distance*cos(angle))<maxHypo*cos(maxAngle) && angle<maxAngle)||(distance<size)){
